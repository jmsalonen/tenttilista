...
const App = () => {
  ...
  useEffect(() => {
    ...
    /* const getUserData = async () => {
      axios
        .get("http://localhost:3001/user/" + user + '/exam')
        .then(response => {
          setUserData(response.data)
      })
    } */
    const getAppData = async () => {
      let dataUrl = "http://localhost:3001/user/" + user + '/exam'
      axios
        .get(dataUrl)
        .then(response => {
  ...
  if (user.length < 1) 
    return (
      <div>
        ...
        <div className="Tenttilista">
          {userData.map(item => <div onClick={() => setUser(item.id)}>{item.name}</div>)}
          <div>{user}</div>
        </div>
      </div>
    )
    ...
    return (
      <div>
        <Header />
        <div className="Tenttilista">
          {appData.map(item => <div>{item.name}</div>)}
        </div>
      </div>
    )
}
